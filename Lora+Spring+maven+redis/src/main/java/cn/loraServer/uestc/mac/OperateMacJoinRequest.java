...
import cn.loraServer.uestc.aes.*;
...
public class OperateMacJoinRequest implements OperateMacPkt {
	...
	public	MacPktForm MacParseData(byte[] data) {
		...
		byte[] datain = Arrays.copyOfRange(data, 0, 18);
		
		byte[] mic = LoRaMacCrypto.LoRaMacJoinComputeMic(datain, datain.length, LoRaMacCrypto.APPSKEY);
		if(mic == data)
